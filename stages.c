...
void p1_place_carrier(board* p1Ships, ship* p1_carrier) {
    ...
    // take the locations of the hulls (possibly turn into a function?)
    do {    
        ...
        // change ship placed to true if its a valid ship (check if interfering with length, orientation, or overlap)    
    ...
    // will only fire IF SHIP IS VALID
    // create ship struct for already declared p1_carrier of specified health and locations
    *p1_carrier = create_ship(5, startRow, startCol, endRow, endCol);
    // change the board by drawing the valid ship (modifies p1Ships)          CODE THIS FUNCTION
